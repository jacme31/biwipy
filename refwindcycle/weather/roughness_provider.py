...
def _sanitize_geospatial_env(verbose: bool = False) -> None:
    ...
    if verbose and actions:
        logger.info("Geospatial env normalized: %s", ", ".join(actions))
...
class RoughnessProvider:
    ...
    def prepare(
        ...
        if _env_flag("DISABLE_RASTERIO"):
            if verbose:
                logger.warning("Roughness provider disabled by DISABLE_RASTERIO=1.")
            return False
        ...
        if has_conflict:
            ...
            if verbose:
                logger.warning("%s. Falling back to default z0=%s m.", reason, Z0_DEFAULT)
                logger.warning("Set FORCE_RASTERIO=1 to bypass this guard and retry.")
            return False
        ...
        try:
            ...
            if verbose:
                logger.warning("Roughness map unavailable (%s). Falling back to default z0=%s m.", exc, Z0_DEFAULT)
            return False
    ...
    def _get_or_download(
        ...
        if existing is not None:
            if verbose:
                logger.info("Roughness raster found in cache: %s", existing.name)
            return existing
    ...
    def _download_cog_window(
        ...
        if dest_path.exists():
            ...
            if verbose:
                logger.info("Roughness raster already present on disk: %s (index repaired)", dest_path.name)
            return dest_path

        if verbose:
            logger.info(
                "Downloading roughness raster from ESA WorldCover...\n"
                "  bbox: W=%.2f S=%.2f E=%.2f N=%.2f\n"
                "  -> %s",
                west,
                south,
                east,
        ...
        if verbose:
            logger.info("  Saved (%.0f KB), registered in index.", filesize / 1024)
    ...
    def _load_into_memory(
        ...
        if verbose:
            logger.info(
                "  z0 map loaded: %sx%s px, z0 range [%.4f - %.4f] m",
                lc_data.shape[1],
                lc_data.shape[0],
                float(z0_data.min()),
                float(z0_data.max()),

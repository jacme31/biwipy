...
class RouteAnalyzer:
    ...
    - load_gpx(): load points
    - segments_from_gpx(): build segments (with smoothing)
    - merge_segments(): merge short segments
    - moving_average_kmh(): moving average speed
    - filter_stops(): remove stops (v < threshold)

    Full workflow (convenience):
    - process_gpx(): wraps the full GPX cleaning pipeline
    """
    ...
    def process_gpx(
        ...
        if verbose:
            print(f"\n{'='*70}")
            print(f"  NETTOYAGE GPX: {gpx_file}")
            print(f"{'='*70}")
            print("[1/6] Chargement du fichier GPX...")
        points = self.load_gpx(gpx_file)

        if verbose:
            print(f"[2/6] Création des segments (smooth={smooth})...")
        ...
        if verbose:
            print(f"      {len(segments)} segments créés, denivelé: {stats['deniv_pos_initial']:.1f} m")

        stats["gps_noise_count"] = 0
        stats["gps_noise_removed"] = 0
        if detect_noise:
            if verbose:
                print("[3/6] Détection des bruits GPS...")
            ...
            if gps_noise and remove_noise:
                if verbose:
                    print(f"      ⚠ {len(gps_noise)} segments aberrants détectés")
                    print("      Suppression des bruits...")
                ...
                if verbose:
                    print(f"      ⚠ {len(gps_noise)} segments aberrants détectés (non supprimés)")
                _push_stage("after_noise_detected_not_removed", {"gps_noise_detected": len(gps_noise)})
            else:
                if verbose:
                    print("      ✓ Aucun segment aberrant détecté")
                _push_stage("after_noise_detection_no_changes")
        else:
            if verbose:
                print("[3/6] Détection bruit GPS: désactivée")
            _push_stage("after_noise_step_disabled")

        stats["merged_count"] = 0
        if merge_segments_flag:
            if verbose:
                print(f"[4/6] Fusion des segments courts (min_dist={self.merge_min_distance}m)...")
            ...
            if verbose:
                print(f"      {n_merged} segments fusionnés")
        else:
            if verbose:
                print("[4/6] Fusion segments: désactivée")
            _push_stage("after_merge_step_disabled")

        stats["stopped_count"] = 0
        if filter_stops_flag:
            if verbose:
                print("[5/6] Filtrage des segments arrêtés (v < 1 m/s)...")
            try:
                ...
                if verbose:
                    print(f"      {stats['stopped_count']} segments arretes filtres")
            except (TypeError, KeyError):
                if verbose:
                    print("      ⚠ Filtrage des arrêts ignoré (timestamps GPX manquants)")
                stats["stopped_count"] = 0
                _push_stage("after_filter_stops_skipped")
        else:
            if verbose:
                print("[5/6] Filtrage arrêts: désactivé")
        ...
        if verbose:
            print("[6/6] Résumé du traitement:")
            print(f"      Segments: {stats['segment_count_initial']} -> {stats['segment_count_final']}")
            print(f"      Distance: {stats['distance_km_initial']:.3f}km -> {stats['distance_km_final']:.3f}km")
            print(f"      Denivelé: {stats['deniv_pos_initial']:.1f}m -> {stats['deniv_pos_final']:.1f}m")
            if stats["rectangle_SN"] is not None and stats["rectangle_EW"] is not None:
                print(f"      Rectangle SN: {stats['rectangle_SN']}, EW: {stats['rectangle_EW']}")
            if has_timestamps:
                print(f"      Timestamps: {t_start.strftime('%Y-%m-%d %H:%M:%S %Z')} → durée: {duration}")
            print(f"{'='*70}\n")
